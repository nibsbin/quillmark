...
### B1 — No `#[non_exhaustive]` in the engine crates
...
### B2 — `serde-saphyr 0.0.23` in `quillmark-core`'s public API
...
### B3 — No declared stability policy
...
### S1 — Internal seams that 1.0 would freeze for the world
...
### S2 — The backend extension point is doc-hidden
...
### S3 — One YAML entry point skips the depth budget
...
### S4 — Library warnings go to stderr and vanish
...
### S5 — Fuzzing misses the decode lanes the bindings expose
...
### S6 — Published crates carry no license text
...
### S8 — No security policy, no advisory scanning
...
- **No render timeout or cancellation.** `Quillmark::render` and
