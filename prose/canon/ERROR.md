...
Three outcomes, and the wire tells them apart only with this table in hand — "no keys" reads identically for the second and third:

- **structured** — keys present; the consumer writes the whole sentence. - **code-determined** — no keys needed; the sentence follows from `code` and the anchor alone. Fully localizable.
