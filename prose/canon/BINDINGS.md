...
`reader.getContent` is the same read at the other end of the codec, returning the **corpus** rather than the projection. It is the read that makes a content field's storage form an implementation detail: the typed write commits a canonical corpus and a markdown parse leaves the authored string (coercion only reconciles them at render), so the transport read answers "corpus or string?" with "depends how this document was built". Decoding needs the schema and not the payload: a `richtext` string is markdown and a `plaintext` string is literal text, so the same stored bytes decode two ways and only the declared type says which. That is the whole reason the corpus read binds the quill instead of sitting beside `getStored`: a quill-free version would have to guess a codec, and would guess markdown.
