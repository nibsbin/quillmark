...
A **whole-document** verb lives on the quill rather than on either cursor: `quill.resolve` and `quill.project` read one. The whole-document *write* stays on the writer, where a typed write already is: `writer.setValues`.
...
| **set** | typed | the writer's strict commit at the write. `set` / `set_all` / `set_values` widen it from one field to one card's fields to the whole document |
...
| Whole-document read | `quill.project(&doc)` → `DocumentValues` | `quill.project(doc)` → a JS object (JS) / `dict` (py) | **identical**: the portable values shape — declared fields the document carries, content leaves as their codec's text, bodies as markdown, `$ext` where present. Sparse and total; never raises. A projection, never a storage format ([SCHEMAS.md](SCHEMAS.md) § "The portable values shape"). `resolve` is the render view beside it, and stays WASM-only |
| Whole-document write | `writer.set_values(&values)?` → `Vec<(DocPath, EditError)>` on refusal | `writer.setValues(values)` (JS), `writer.set_values(values)` (py) | **identical**: replace, not merge; all-or-nothing, one diagnostic per refused cell carrying its own `DocPath` (`main.qty`, `cards.line_item[0].desc`). A cell equal to its projection is not written, so writing back an unedited `project` is a byte no-op. A values object the binding cannot read as the shape throws / raises `ValueError` before any of it applies |
...
> **Scope: Tier 1 + storage + render.** Field I/O flows through `quill.writer(doc)` / `quill.reader(doc)` exclusively, plus the whole-document `quill.project(doc)` / `writer.set_values(values)`; `Document` is quill-free data and structure (parse, the storage DTO, `insert_card` / `remove_card` / `move_card` / `make_card`, `remove_field`, the `card` / `seed_overlay` reads, `$ext` / `$seed`).
>
