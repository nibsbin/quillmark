...
| Nested `Content` read | `reader.get_content_at(name, path)?`; `reader.card(i)?.get_content_at(..)?` | `reader.getContentAt(addr, path)` / `reader.card(i).getContentAt(name, path)` (JS), `reader.get_content_at(name, path)` / `reader.card(i).get_content_at(name, path)` (py) | **idiom** / **FFI**: the same read one axis in, `path` a `PathStep[]` / `Sequence[str \| int]` walked through the field schema's `items` / `properties` to the leaf whose declared type names the codec. The empty path *is* `get_content`. A path naming nothing stored (a stale row index, a key the object lacks) reads absent rather than throwing — the field axis's totality applied to the axis a repeater mutates; a bad *card* index still throws, being guarded by a count the caller holds. The path is the read's own argument and not an `Addr` member: `Addr` is the currency of every `Document` verb, and `storeField` / `isFill` / `applyChange` cannot answer an element axis |
