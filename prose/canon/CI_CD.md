...
| Job | What it does |
|-----|-------------|
| `lint` | `node scripts/check-canon.mjs` (the canon spine and link gate — see [prose/README.md](../README.md)), then `cargo doc --no-deps --locked` with `RUSTDOCFLAGS=-Dwarnings` — the standing lint gate; clippy is deliberately not gated. That command walks default-members, so `quillmark-wasm` and `quillmark-content` get a `-p` step each: both sit outside default-members and neither was linted until broken intra-doc links accumulated in them. Also asserts `scripts/strip-seed-comment.sh` strips a seed block and no-ops without one: `release.yml` runs it unattended against `CHANGELOG.md` between the version bump and the tag, so a regression corrupts a release commit. It rides here rather than in its own job because the assertions take 0s and a runner for them was pure overhead |
