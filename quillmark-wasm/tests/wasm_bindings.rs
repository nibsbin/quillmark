...
fn test_usaf_form_8_glue_output() {
    use quillmark_core::{Quill, ParsedDocument};
    use std::path::Path;
    ...
    let json: serde_json::Value = serde_json::from_str(&glue_output)
        .expect("Failed to parse glue output as JSON");
    ...
    assert_eq!(first.unwrap().as_str(), Some("Phillip"), "examinee.first should be 'Phillip'");
    ...
    println!("Glue output (first 500 chars): {}", &glue_output[..glue_output.len().min(500)]);
}
...
fn quill_to_json(quill_path: &str) -> String {
    ...
    fn build_file_tree(path: &Path, base_path: &Path) -> serde_json::Value {
        ...
        if path.is_file() {
            ...
            let is_text = path.extension().and_then(|e| e.to_str()).map_or(true, |ext| {
                matches!(ext, "toml" | "md" | "txt" | "typ" | "tex" | "html" | "css" | "js")
            });
...
fn test_usaf_form_8_from_json_matches_from_path() {
    ...
    println!("Quill JSON (first 1000 chars): {}", &quill_json[..quill_json.len().min(1000)]);
    ...
    assert!(form_pdf_from_path.is_some(), "form.pdf not found in quill from path");
    assert!(form_pdf_from_json.is_some(), "form.pdf not found in quill from JSON");
    assert_eq!(form_pdf_from_path.unwrap().len(), form_pdf_from_json.unwrap().len(), 
...
fn test_usaf_form_8_render_via_json_workflow() {
    use quillmark_core::{Quill, ParsedDocument, OutputFormat};
    use std::path::Path;
    ...
    assert_eq!(examinee.get("first").and_then(|v| v.as_str()), Some("Phillip"));
    assert_eq!(examinee.get("last").and_then(|v| v.as_str()), Some("Fry"));
    ...
    println!("Successfully rendered PDF with {} bytes", result.artifacts[0].bytes.len());
}
