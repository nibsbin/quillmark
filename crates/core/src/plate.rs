//! Plate template bundle types and implementations.
...
pub struct UiSchema {
    ...
    /// Order of the field in the UI (automatically generated based on field position in Plate.toml)
...
impl FieldSchema {
    ...
    pub fn from_plate_value(key: String, value: &QuillValue) -> Result<Self, String> {
        let obj = value
            .as_object()
        ...
        let ui = if let Some(ui_value) = obj.get("ui") {
            if let Some(ui_obj) = ui_value.as_object() {
                ...
                Some(UiSchema {
                    group,
                    tooltip,
                    order: None, // Order is determined by position in Plate.toml
                })
            } else {
...
/// Simple gitignore-style pattern matcher for .plateignore
#[derive(Debug, Clone)]
pub struct PlateIgnore {
    patterns: Vec<String>,
}

impl PlateIgnore {
    /// Create a new PlateIgnore from pattern strings
    pub fn new(patterns: Vec<String>) -> Self {
        Self { patterns }
    }

    /// Parse .plateignore content into patterns
...
/// A plate template bundle.
#[derive(Debug, Clone)]
pub struct Plate {
    /// Plate-specific metadata
...
/// Plate configuration extracted from Plate.toml
#[derive(Debug, Clone)]
pub struct PlateConfig {
    ...
    /// Additional metadata from [Plate] section (excluding standard fields)
    pub metadata: HashMap<String, QuillValue>,
...
impl PlateConfig {
    /// Parse PlateConfig from TOML content
    pub fn from_toml(toml_content: &str) -> Result<Self, Box<dyn StdError + Send + Sync>> {
        let plate_toml: toml::Value = toml::from_str(toml_content)
            .map_err(|e| format!("Failed to parse Plate.toml: {}", e))?;
        ...
        // Extract [Plate] section (required)
        let plate_section = plate_toml
            .get("Plate")
            .ok_or("Missing required [Plate] section in Plate.toml")?;

        // Extract required fields
        let name = plate_section
            .get("name")
            .and_then(|v| v.as_str())
            .ok_or("Missing required 'name' field in [Plate] section")?
            .to_string();

        let backend = plate_section
            .get("backend")
            .and_then(|v| v.as_str())
            .ok_or("Missing required 'backend' field in [Plate] section")?
            .to_string();

        let description = plate_section
            .get("description")
            .and_then(|v| v.as_str())
            .ok_or("Missing required 'description' field in [Plate] section")?;

        if description.trim().is_empty() {
            return Err("'description' field in [Plate] section cannot be empty".into());
        }
        let description = description.to_string();

        // Extract optional fields
        let version = plate_section
            .get("version")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());

        let author = plate_section
            .get("author")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());

        let example_file = plate_section
            .get("example_file")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());

        let glue_file = plate_section
            .get("glue_file")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());

        // Extract additional metadata from [Plate] section (excluding standard fields)
        let mut metadata = HashMap::new();
        if let toml::Value::Table(table) = plate_section {
        ...
        if let Some(typst_section) = plate_toml.get("typst") {
        ...
        if let Some(fields_section) = plate_toml.get("fields") {
            if let toml::Value::Table(fields_table) = fields_section {
                ...
                for (field_name, field_schema) in fields_table {
                    ...
                    match QuillValue::from_toml(field_schema) {
                        Ok(quill_value) => {
                            match FieldSchema::from_plate_value(field_name.clone(), &quill_value) {
                                Ok(mut schema) => {
                                    // Always set ui.order based on position in Plate.toml
...
impl Plate {
    /// Create a Plate from a directory path
    pub fn from_path<P: AsRef<std::path::Path>>(
        ...
        // Load .plateignore if it exists
        let plateignore_path = path.join(".plateignore");
        let ignore = if plateignore_path.exists() {
            let ignore_content = fs::read_to_string(&plateignore_path)
                .map_err(|e| format!("Failed to read .plateignore: {}", e))?;
            PlateIgnore::from_content(&ignore_content)
        } else {
            // Default ignore patterns
            PlateIgnore::new(vec![
                ".git/".to_string(),
                ".gitignore".to_string(),
                ".plateignore".to_string(),
                "target/".to_string(),
        ...
        // Create Plate from the file tree
        Self::from_tree(root, Some(name))
    }

    /// Create a Plate from a tree structure
    ///
    /// This is the authoritative method for creating a Plate from an in-memory file tree.
    ///
    /// # Arguments
    ///
    /// * `root` - The root node of the file tree
    /// * `_default_name` - Unused parameter kept for API compatibility (name always from Plate.toml)
    ///
    /// # Errors
    ///
    /// Returns an error if:
    /// - Plate.toml is not found in the file tree
    /// - Plate.toml is not valid UTF-8 or TOML
    /// - The glue file specified in Plate.toml is not found or not valid UTF-8
    /// - Validation fails
    pub fn from_tree(
        root: FileTreeNode,
        ...
        // Read Plate.toml
        let plate_toml_bytes = root
            .get_file("Plate.toml")
            .ok_or("Plate.toml not found in file tree")?;

        let plate_toml_content = String::from_utf8(plate_toml_bytes.to_vec())
            .map_err(|e| format!("Plate.toml is not valid UTF-8: {}", e))?;

        // Parse TOML into PlateConfig
        let config = PlateConfig::from_toml(&plate_toml_content)?;

        // Construct Plate from PlateConfig
        Self::from_config(config, root)
    }

    /// Create a Plate from a PlateConfig and file tree
    ///
    /// This method constructs a Plate from a parsed PlateConfig and validates
    /// all file references.
    ///
    /// # Arguments
    ///
    /// * `config` - The parsed PlateConfig
    /// * `root` - The root node of the file tree
    ///
    /// # Errors
    ///
    ...
    /// Create a Plate from a JSON representation
    ///
    /// Parses a JSON string into an in-memory file tree and validates it. The
    /// precise JSON contract is documented in `designs/PLATE.md`.
    ...
    /// Get the list of typst packages to download, if specified in Plate.toml
    ...
    /// during Plate construction. This is more efficient than re-parsing the schema.
    ///
    ...
    /// Get file contents by path (relative to plate root)
    pub fn get_file<P: AsRef<Path>>(&self, path: P) -> Option<&[u8]> {
    ...
    /// List all files in a directory (returns paths relative to plate root)
    ...
    /// List all directories in a directory (returns paths relative to plate root)
...
.git/
"#;
        let ignore = PlateIgnore::from_content(ignore_content);
    ...
    #[test]
    fn test_plateignore_matching() {
        let ignore = PlateIgnore::new(vec![
            "*.tmp".to_string(),
            "target/".to_string(),
            "node_modules/".to_string(),
            ".git/".to_string(),
    ...
    fn test_in_memory_file_system() {
        ...
        let plate_dir = temp_dir.path();

        // Create test files
        fs::write(
            plate_dir.join("Plate.toml"),
            "[Plate]\nname = \"test\"\nbackend = \"typst\"\nglue_file = \"glue.typ\"\ndescription = \"Test plate\"",
        )
        .unwrap();
        fs::write(plate_dir.join("glue.typ"), "test glue").unwrap();

        let assets_dir = plate_dir.join("assets");
        fs::create_dir_all(&assets_dir).unwrap();
        fs::write(assets_dir.join("test.txt"), "asset content").unwrap();

        let packages_dir = plate_dir.join("packages");
        fs::create_dir_all(&packages_dir).unwrap();
        fs::write(packages_dir.join("package.typ"), "package content").unwrap();

        // Load plate
        let plate = Plate::from_path(plate_dir).unwrap();

        // Test file access
        assert!(plate.file_exists("glue.typ"));
        assert!(plate.file_exists("assets/test.txt"));
        assert!(plate.file_exists("packages/package.typ"));
        assert!(!plate.file_exists("nonexistent.txt"));

        // Test file content
        let asset_content = plate.get_file("assets/test.txt").unwrap();
        assert_eq!(asset_content, b"asset content");

        // Test directory listing
        let asset_files = plate.list_directory("assets");
        assert_eq!(asset_files.len(), 1);
        assert!(asset_files.contains(&PathBuf::from("assets/test.txt")));
    }

    #[test]
    fn test_plateignore_integration() {
        let temp_dir = TempDir::new().unwrap();
        let plate_dir = temp_dir.path();

        // Create .plateignore
        fs::write(plate_dir.join(".plateignore"), "*.tmp\ntarget/\n").unwrap();
        ...
        .unwrap();
        fs::write(plate_dir.join("glue.typ"), "test template").unwrap();
        fs::write(plate_dir.join("should_ignore.tmp"), "ignored").unwrap();

        let target_dir = plate_dir.join("target");
        ...
        assert!(plate.file_exists("glue.typ"));
        assert!(!plate.file_exists("should_ignore.tmp"));
        assert!(!plate.file_exists("target/debug.txt"));
    }
    ...
    fn test_find_files_pattern() {
        ...
        .unwrap();
        fs::write(plate_dir.join("glue.typ"), "template").unwrap();
        ...
        let all_assets = plate.find_files("assets/*");
        assert!(all_assets.len() >= 3); // At least image.png, data.json, fonts/font.ttf

        let typ_files = plate.find_files("*.typ");
    ...
    fn test_new_standardized_toml_format() {
        ...
        let toml_content = r#"[Plate]
name = "my-custom-plate"
backend = "typst"
glue_file = "custom_glue.typ"
description = "Test plate with new format"
author = "Test Author"
"#;
        fs::write(plate_dir.join("Plate.toml"), toml_content).unwrap();
        fs::write(
            plate_dir.join("custom_glue.typ"),
        ...
        assert_eq!(plate.name, "my-custom-plate");

        // Test that backend is in metadata
        assert!(plate.metadata.contains_key("backend"));
        if let Some(backend_val) = plate.metadata.get("backend") {
        ...
        assert!(plate.metadata.contains_key("description"));
        assert!(plate.metadata.contains_key("author"));
        assert!(!plate.metadata.contains_key("version")); // version should be excluded

        // Test that glue template content is loaded correctly
        assert!(plate.glue.unwrap().contains("Custom Template"));
    }
...
backend = "typst"
glue_file = "glue.typ"
description = "Test plate for packages"
...
"#;
        ...
        fs::write(plate_dir.join("Plate.toml"), toml_content).unwrap();
        fs::write(plate_dir.join("glue.typ"), "test").unwrap();

        let plate = Plate::from_path(plate_dir).unwrap();
        let packages = plate.typst_packages();
...
description = "Test plate with template"
"#;
        fs::write(plate_dir.join("Plate.toml"), toml_content).unwrap();
        fs::write(plate_dir.join("glue.typ"), "glue content").unwrap();
        fs::write(
            plate_dir.join("example.md"),
...
backend = "typst"
glue_file = "glue.typ"
description = "Test plate without template"
"#;
        fs::write(plate_dir.join("Plate.toml"), toml_content).unwrap();
        fs::write(plate_dir.join("glue.typ"), "glue content").unwrap();
    ...
    fn test_from_tree() {
        ...
        // Add Plate.toml
        let plate_toml = r#"[Plate]
name = "test-from-tree"
backend = "typst"
glue_file = "glue.typ"
description = "A test plate from tree"
"#;
        root_files.insert(
            "Plate.toml".to_string(),
            FileTreeNode::File {
                contents: plate_toml.as_bytes().to_vec(),
            },
        );
        ...
        // Create Plate from tree
        let plate = Plate::from_tree(root, Some("test-from-tree".to_string())).unwrap();

        // Validate the plate
        assert_eq!(plate.name, "test-from-tree");
        assert_eq!(plate.glue.unwrap(), glue_content);
        assert!(plate.metadata.contains_key("backend"));
        assert!(plate.metadata.contains_key("description"));
    }
    ...
    fn test_from_tree_with_template() {
        ...
        // Add Plate.toml with example specified
        let plate_toml = r#"[Plate]
...
"#;
        ...
        // Create Plate from tree
        let plate = Plate::from_tree(root, None).unwrap();

        // Validate template is loaded
        assert_eq!(plate.example, Some(template_content.to_string()));
    }

    #[test]
    fn test_from_json() {
        // Create JSON representation of a Plate using new format
        let json_str = r#"{
            "metadata": {
                "name": "test-from-json"
            },
            "files": {
                "Plate.toml": {
                    "contents": "[Plate]\nname = \"test-from-json\"\nbackend = \"typst\"\nglue_file = \"glue.typ\"\ndescription = \"Test plate from JSON\"\n"
                },
                "glue.typ": {
        ...
        // Create Plate from JSON
        let plate = Plate::from_json(json_str).unwrap();

        // Validate the plate
        assert_eq!(plate.name, "test-from-json");
        assert!(plate.glue.unwrap().contains("Test Glue"));
        assert!(plate.metadata.contains_key("backend"));
    }
    ...
    fn test_from_json_with_byte_array() {
        ...
        let json_str = r#"{
            "files": {
                "Plate.toml": {
                    "contents": [91, 80, 108, 97, 116, 101, 93, 10, 110, 97, 109, 101, 32, 61, 32, 34, 116, 101, 115, 116, 34, 10, 98, 97, 99, 107, 101, 110, 100, 32, 61, 32, 34, 116, 121, 112, 115, 116, 34, 10, 103, 108, 117, 101, 95, 102, 105, 108, 101, 32, 61, 32, 34, 103, 108, 117, 101, 46, 116, 121, 112, 34, 10, 100, 101, 115, 99, 114, 105, 112, 116, 105, 111, 110, 32, 61, 32, 34, 84, 101, 115, 116, 32, 112, 108, 97, 116, 101, 34, 10]
                },
                "glue.typ": {
                    "contents": "test glue"
                }
        ...
        // Validate the plate was created
        assert_eq!(plate.name, "test");
        assert_eq!(plate.glue.unwrap(), "test glue");
    }
    ...
    fn test_from_json_missing_files() {
        ...
        let result = Plate::from_json(json_str);
    ...
    fn test_from_json_tree_structure() {
        ...
        assert_eq!(plate.name, "test-tree-json");
        assert!(plate.glue.unwrap().contains("Tree structure content"));
        assert!(plate.metadata.contains_key("backend"));
    }
    ...
    fn test_from_json_nested_tree_structure() {
        ...
        assert_eq!(plate.name, "nested-test");
        // Verify nested files are accessible
        assert!(plate.file_exists("src/main.rs"));
        assert!(plate.file_exists("src/lib.rs"));

        let main_rs = plate.get_file("src/main.rs").unwrap();
    ...
    fn test_from_tree_structure_direct() {
        ...
        assert_eq!(plate.name, "direct-tree");
        assert!(plate.file_exists("src/main.rs"));
        assert!(plate.file_exists("glue.typ"));
    }
    ...
    fn test_from_json_with_metadata_override() {
        // Test that metadata key overrides name from Plate.toml
        let json_str = r#"{
            "metadata": {
                "name": "override-name"
            },
        ...
        let plate = Plate::from_json(json_str).unwrap();
        // Metadata name should be used as default, but Plate.toml takes precedence
        // when from_tree is called
        assert_eq!(plate.name, "toml-name");
    }
    ...
    fn test_from_json_empty_directory() {
        ...
        let plate = Plate::from_json(json_str).unwrap();
        assert_eq!(plate.name, "empty-dir-test");
        assert!(plate.dir_exists("empty_dir"));
        assert!(!plate.file_exists("empty_dir"));
    }
    ...
    fn test_dir_exists_and_list_apis() {
        ...
        // Test dir_exists
        assert!(plate.dir_exists("assets"));
        assert!(plate.dir_exists("assets/fonts"));
        assert!(plate.dir_exists("empty"));
        assert!(!plate.dir_exists("nonexistent"));
        assert!(!plate.dir_exists("glue.typ")); // file, not directory

        // Test file_exists
        assert!(plate.file_exists("glue.typ"));
        assert!(plate.file_exists("assets/logo.png"));
        assert!(plate.file_exists("assets/fonts/font.ttf"));
        assert!(!plate.file_exists("assets")); // directory, not file

        // Test list_files
        let root_files_list = plate.list_files("");
        assert_eq!(root_files_list.len(), 2); // Plate.toml and glue.typ
        assert!(root_files_list.contains(&"Plate.toml".to_string()));
        assert!(root_files_list.contains(&"glue.typ".to_string()));

        let assets_files_list = plate.list_files("assets");
        ...
        let root_subdirs = plate.list_subdirectories("");
        ...
        let assets_subdirs = plate.list_subdirectories("assets");
        assert_eq!(assets_subdirs.len(), 1); // fonts
        assert!(assets_subdirs.contains(&"fonts".to_string()));

        let empty_subdirs = plate.list_subdirectories("empty");
    ...
    fn test_field_schemas_parsing() {
        ...
        // Add Plate.toml with field schemas
        let plate_toml = r#"[Plate]
name = "taro"
backend = "typst"
glue_file = "glue.typ"
...
"#;
        ...
        // Create Plate from tree
        let plate = Plate::from_tree(root, Some("taro".to_string())).unwrap();

        // Validate field schemas were parsed
        assert_eq!(plate.schema["properties"].as_object().unwrap().len(), 3);
        assert!(plate.schema["properties"]
            .as_object()
            .unwrap()
            .contains_key("author"));
        assert!(plate.schema["properties"]
        ...
        let author_schema = plate.schema["properties"]["author"].as_object().unwrap();
        ...
        let ice_cream_schema = plate.schema["properties"]["ice_cream"].as_object().unwrap();
        assert_eq!(ice_cream_schema["description"], "favorite ice cream flavor");

        // Verify title field schema
        let title_schema = plate.schema["properties"]["title"].as_object().unwrap();
...
"#;
        ...
        let schema2 = FieldSchema::from_plate_value("test_name".to_string(), &quill_value).unwrap();
    ...
    #[test]
    fn test_plate_without_glue_file() {
        // Test creating a Plate without specifying a glue file
        let mut root_files = HashMap::new();

        // Add Plate.toml without glue field
        let plate_toml = r#"[Plate]
name = "test-no-glue"
backend = "typst"
description = "Test plate without glue file"
"#;
        root_files.insert(
            "Plate.toml".to_string(),
            FileTreeNode::File {
        ...
        assert!(plate.glue.clone().is_none());
        assert_eq!(plate.name, "test-no-glue");
    }

    #[test]
    fn test_plate_config_from_toml() {
        // Test parsing PlateConfig from TOML content
        let toml_content = r#"[Plate]
name = "test-config"
backend = "typst"
...
"#;
        ...
        let config = PlateConfig::from_toml(toml_content).unwrap();
    ...
    #[test]
    fn test_plate_config_missing_required_fields() {
        // Test that missing required fields result in error
        let toml_missing_name = r#"[Plate]
backend = "typst"
description = "Missing name"
"#;
        let result = PlateConfig::from_toml(toml_missing_name);
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
        ...
        let toml_missing_backend = r#"[Plate]
name = "test"
description = "Missing backend"
"#;
        let result = PlateConfig::from_toml(toml_missing_backend);
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
        ...
        let toml_missing_description = r#"[Plate]
name = "test"
backend = "typst"
"#;
        let result = PlateConfig::from_toml(toml_missing_description);
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
    ...
    #[test]
    fn test_plate_config_empty_description() {
        // Test that empty description results in error
        let toml_empty_description = r#"[Plate]
name = "test"
backend = "typst"
description = "   "
"#;
        let result = PlateConfig::from_toml(toml_empty_description);
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("description' field in [Plate] section cannot be empty"));
    }

    #[test]
    fn test_plate_config_missing_plate_section() {
        // Test that missing [Plate] section results in error
        let toml_no_section = r#"[fields]
title = {description = "Title"}
"#;
        let result = PlateConfig::from_toml(toml_no_section);
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Missing required [Plate] section"));
    }

    #[test]
    fn test_plate_from_config_metadata() {
        // Test that PlateConfig metadata flows through to Plate
        let mut root_files = HashMap::new();
...
"#;
        ...
        assert!(plate.metadata.contains_key("custom_field"));
        assert_eq!(
            plate.metadata.get("custom_field").unwrap().as_str(),
            Some("custom_value")
        );

        // Verify typst config with typst_ prefix
        assert!(plate.metadata.contains_key("typst_packages"));
    }

    #[test]
    fn test_extract_defaults_method() {
        // Test the extract_defaults method on Plate
        let mut root_files = HashMap::new();
...
"#;
    ...
    #[test]
    fn test_plate_with_all_ui_properties() {
        let toml_content = r#"[Plate]
name = "full-ui-test"
backend = "typst"
