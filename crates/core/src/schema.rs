...
use crate::parse::CARD_LABEL_FIELD;
...
fn build_card_def(name: &str, card: &CardSchema) -> Map<String, Value> {
    ...
    // Add CARD_LABEL property (auto-generated by parser)
    let mut label_prop = Map::new();
    label_prop.insert("type".to_string(), Value::String("string".to_string()));
    label_prop.insert(
        "description".to_string(),
        Value::String(
            "Auto-generated display label for the card instance (e.g. \"items 1\")".to_string(),
        ),
    );
    properties.insert(CARD_LABEL_FIELD.to_string(), Value::Object(label_prop));
