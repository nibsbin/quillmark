...
"#
    ...
    // Render plate output (JSON)
    let print_output = workflow.render_plate(&parsed)?;
    write_example_output("usaf_form_8_print.json", print_output.as_bytes())?;

    let output_dir = PathBuf::from("crates/fixtures/output/");

    println!(
        "Wrote print output to examples output directory: {}",
        output_dir.join("usaf_form_8_print.json").display()
    );
