...
//! delimiters verbatim. That sharing extends to geometry — plaintext is a
//! content field, scalar or array element, so it surfaces regions and answers
//! `position_at` / `locate` — and these run at engine altitude because the
//! inheritance spans two links: the render floor coerces the resting literal
//! string to a content object, and only then does the backend classify it.
...
/// Fields authored the way a card holds them: `plaintext` rests as its literal
/// string, so these go in unparsed and ride the floor's coercion.
...
fn plaintext_elements_and_scalars_surface_navigable_regions() {
    ...
    // One region per element, keyed `<field>.<i>` and carrying a content span;
    // the array itself names none, its ink being its elements'.
    for (field, len) in [
    ...
    // A click inside an element addresses *that element*, cluster-exact.
    ...
    // And round-trips: the caret for that offset sits inside the element.
...
fn a_plaintext_delimiter_is_one_addressable_cluster() {
    ...
    // The literal codec lowers `*` to `\*`, two generated bytes for one content
    // char. Carets stay ordered across the escape, and a click on the glyph
    // floors to the char rather than leaking the escape's second byte.
    let caret_x = |pos: usize| session.locate("tags.0", pos).expect("caret").rect[0];
    let mut prev = f32::MIN;
    // Content chars only: the exclusive end is a segment edge, and `locate`
    // falls back to the segment start there for every codec.
