...
fn test_with_asset_basic() {
    ...
    workflow.add_asset("taro.png", taro_picture).expect("Should add asset");
...
fn test_with_asset_collision() {
    ...
    let quill = engine.quill_from_path(quills_path("taro")).unwrap();
    let mut workflow = engine.workflow(&quill).unwrap();
    workflow.add_asset("taro.png", vec![1, 2, 3]).expect("Should add first asset");
...
fn test_with_assets_multiple() {
    ...
    let mut workflow = engine.workflow(&quill).unwrap();
    workflow.add_assets(assets).expect("Should add multiple assets");
    assert!(workflow.quill_ref().starts_with("taro@"));
}
...
fn test_clear_assets() {
    ...
    workflow.add_asset("taro.png", vec![7, 8, 9]).expect("Should add again after clearing");
    workflow.add_asset("more_taro.png", vec![10, 11, 12]).expect("Should add again after clearing");
    assert!(workflow.quill_ref().starts_with("taro@"));
}
