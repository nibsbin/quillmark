...
impl Workflow {
    ...
    pub fn render(
        ...
        let print = self.render_plate(parsed)?;

        // Prepare quill with dynamic assets
        let prepared_quill = self.prepare_quill_with_assets();

        // Pass prepared quill to backend
        self.render_processed_with_quill(&print, format, &prepared_quill)
    }

    /// Render pre-processed print content, skipping parsing and plate rendering.
    ...
    /// Render a parsed document through the plate template to produce a print
    pub fn render_plate(&self, parsed: &ParsedDocument) -> Result<String, RenderError> {
