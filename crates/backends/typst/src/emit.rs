...
//! records, per segment, the generated byte window plus one `(content ↔ generated)`
...
//!
//! Each run records only its `(content, generated)` pair; per-char spans are
...
/// `i` of `generated`; mirrors [`escape_markup`]/[`escape_string`], kept aligned by
...
/// run — cluster floor. `generated` is the run's generated slice, `target` a byte
...
/// the run's content start; the returned byte offset is into `generated`. An offset
