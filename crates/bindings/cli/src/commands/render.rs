...
pub struct RenderArgs {
    ...
    /// Only render the plate template, don't compile final output
    #[arg(long)]
    plate_only: bool,
...
pub fn execute(args: RenderArgs) -> Result<()> {
    ...
    if args.plate_only {
        if args.verbose {
            println!("Rendering plate template...");
        }
        ...
        let output_path = args.output.unwrap_or_else(|| {
            if let Some(ref path) = markdown_path_for_output {
                derive_print_output_path(path)
            } else {
                PathBuf::from("example_print.typ")
            }
        });

        let writer = OutputWriter::new(args.stdout, Some(output_path), args.quiet);
        writer.write(&print_bytes)?;
