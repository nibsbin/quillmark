...
fn test_metadata_retrieval() {
    ...
    engine.register_quill(JsValue::from_str(UI_QUILL_JSON)).map_err(|e| {
        let error_obj: Value = serde_wasm_bindgen::from_value(e).unwrap();
        panic!("register failed: {:#?}", error_obj);
    }).unwrap();

    let info_js = engine.get_quill_info("ui-test-quill").expect("getQuillInfo failed");
    ...
    let x_ui = info.pointer("/schema/properties/my_field/x-ui")
