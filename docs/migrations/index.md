...
| Step | What changes |
|---|---|
| [0.102 → 0.103](0.102-to-0.103.md) | WASM-only. `init()` resolves to the core surface instead of `void`, and `Quill`, `Document` and the free functions leave the static exports: destructure them from `await init()`. The gate becomes the only way to obtain one, so the pre-init mistake stops being expressible and `runtime::not_initialized` retires with the door it guarded. `Engine`, `MAIN_CARD_ADDR`, `isQuillmarkError` and the open-set guards are unchanged, as are the `Quill` / `Document` **type** exports. Documents, stored blobs, Rust and Python are untouched. |
