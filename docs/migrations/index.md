...
| Step | What changes |
|---|---|
| [0.104 → 0.105](0.104-to-0.105.md) | A card's declared `string`/`number` fields reach a Typst plate as the two-projection value-object `date` fields already used: `(card.from.display)()` renders ink carrying a per-card region, `card.from.value` is the raw value for packages and string work. It buys click-to-edit on card scalars, which previously needed a `form-field` bound to them. Root fields, array elements, booleans, `none`, undeclared keys and `$kind` dispatch are unchanged. Plates that read a card scalar need edits; one break — comparing a bare cell, `card.title != ""` — renders wrongly instead of failing the compile. Documents, stored blobs, and every non-Typst surface are untouched. |
