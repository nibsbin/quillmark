...
The content fixed point is unchanged: import is newline-insensitive, so a
re-parse of either form yields the same content.
...
**The rename is free for the values it assumes, and the corpus decides whether
your field is that case.** The old unified type accepted all three kinds in one
field, so a field the schema documented as `YYYY-MM-DD` may still hold a wall
clock written by hand, by an API caller, or by a control that stamped a time.
Audit before the schema edit ships — see [Audit the corpus first](#audit-the-corpus-first)
below.
...
- Rename every `type: datetime` field that holds a calendar date to `type: date`
  (the common case). No document changes, **provided every stored value in that
  field is already bare** — audit first.
...
- **The two grammars are disjoint**: `date` rejects any time component and
  `datetime` rejects a bare date, so no string satisfies both. A field holding a
  mix has no correct declaration — whichever type it takes strands the other
  half. Staying on `type: datetime` is not a safe fallback for a mixed field;
  normalize the values or split the field in two.

### Audit the corpus first

A field that holds bare dates in every document but one migrates cleanly for
every document but one. That one strands: `validation::format_violation` at
`quill.validate`, and a render throw for a consumer that does not call it.
`quill.conform` is **silent** on the case by design — it walks content fields,
and a date is a scalar — so a clean conform is not evidence the rename is safe.

Walk the stored rows through the transport door before publishing the schema
edit. The stored string is never rewritten (neither the transport door nor
`conform` touches a scalar), so the audit is read-only until you repair:

```js
const doc = Document.fromJson(row);
const value = quill.reader(doc).get('issued');   // stored string, verbatim
if (value && !/^\d{4}-\d{2}-\d{2}$/.test(value)) {
  // Strands under `type: date`. Repair the document before the rename ships.
  quill.writer(doc).set('issued', value.slice(0, 10));
}
```

**A plate cannot compensate.** Coercion runs when the document lowers into the
backend's `data` dict, upstream of the plate: a stranded value fails the render
before any template code executes, and a plate that never mentions the field
fails identically. There is no plate-side coercion to write, so the repair has
