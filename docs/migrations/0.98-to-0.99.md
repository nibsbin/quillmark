...
No stored population is affected. A `0.98` writer could not emit an unknown class
— its own decoder had already flattened one to `unrepresentable` — so the new
arm is reachable only from a document a *future* build wrote.
