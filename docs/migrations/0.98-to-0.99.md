...
| `KnownIslandType::ALL` becomes a slice | Rust (`quillmark-content`) | Name the slice type; iterate by reference |
...
`KnownIslandType::ALL` is `&'static [KnownIslandType]` on the same grounds. The
enum is closed, so a third island type already breaks an exhaustive match; the
array pinned its length in the type as well, breaking every caller that names
the constant's type on top of that.
