...
`YamlError` carries `message()` plus optional 1-indexed `line()` / `column()`,
and renders to a `yaml::parse_error` diagnostic via `to_diagnostic(file)` — more
than the old types gave a caller that was not willing to match on the engine's
own variants. Code that only propagated the error with `?` into a `Box<dyn
...
## The public enums open ahead of 1.0.0
...
Four types stay exhaustive on purpose, each saying so in its rustdoc:
`Severity` (the two-value ladder is the contract), `FileTreeNode` (a tree is
files and directories), the `V0_92_0` storage DTOs (a shipped schema version
never changes), and `quillmark_pdf::FieldType` (two out-of-crate sites dispatch
over the whole set, and a variant they miss draws nothing on the page — the
compile error is the guardrail).

`RenderOptions` also stays exhaustive, for a different reason: the attribute
forbids every out-of-crate struct expression, functional update included, so
`RenderOptions { output_format: …, ..Default::default() }` would stop compiling
everywhere. Keep using that idiom and a new field costs you nothing.
