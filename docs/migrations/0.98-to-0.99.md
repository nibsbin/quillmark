...
| Most public enums become `#[non_exhaustive]`; `OutputFormat::ALL` becomes a slice | Rust (`quillmark-core`, `quillmark-content`, `quillmark-pdf`, `quillmark-pdfform`) | Add a `_` arm where you match one; iterate `ALL` by reference |
...
| `Loss` gains `Unknown(String)` and no longer derives `Copy` | Rust (`quillmark-content`) | Clone or borrow where a copy was implicit; read fidelity through `Loss::fidelity` |
| An unrecognized island `loss` class round-trips verbatim instead of decoding to `unrepresentable` | Storage (all surfaces) | Nothing — no `0.98`-written blob carries one |
...
| A `Quill`/`Document` from a second copy of `@quillmark/wasm` is refused everywhere | TypeScript (`Engine`, the writer/reader binds, `equals`/`validate`/`resolve`) | `npm ls @quillmark/wasm` and dedupe to one copy |
...
`Container`, `MarkKind`, `Loss`, `Invariant`, `LineKindMismatch`, `ParseError`,
...
`ChangeSet`, `Diagnostic`, `RenderResult`, `Location`, `CardWire`, `Version`,
`QuillReference`, `RenderOptions`, and `quillmark-pdf`'s `FieldSpec` and
`StampOptions`; plus the schema family a caller assembling a quill in memory
builds — `QuillConfig`, `CardSchema`, `FieldSchema`, `UiCardSchema`,
`UiFieldSchema`, `BodyCardSchema`, and `GroupSchema`. The rest of the sweep
lands on types only ever handed *back* to you — `Resolved*`, `FormSpec`,
`BoundField`, `BoundWidget`, `UnboundWidget`, `Rect`, `Parsed`, `SeedOverlay`,
`NestedComment`, `DocPathParseError`, `PdfError`, `PdfUpdate`, `UpdatedObject` —
where the attribute costs a caller nothing.

Each has a `new` taking the fields it always carries, and a `with_*` setter for
each optional one. A type whose fields are *all* optional starts from
`default()` instead of a `new`: `RenderOptions`, `StampOptions`, `UiCardSchema`,
`UiFieldSchema`, and `BodyCardSchema`.
