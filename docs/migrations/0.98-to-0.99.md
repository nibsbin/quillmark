...
| An opaque payload nesting past 128 levels rejected on decode | TypeScript (`install` / `applyChange` / `makeCard` / `insertCard`), Rust (`serial::from_canonical_value`, the op wire) | Nothing — no storable blob reaches the limit |
...
| `Loss` becomes an opaque string wrapper and no longer derives `Copy` | Rust (`quillmark-content`) | `Loss::Lossless` → `Loss::LOSSLESS`; clone or borrow where a copy was implicit; read fidelity through `Loss::fidelity` |
...
Four stay exhaustive, each saying so in its rustdoc. The storage DTOs
(`V0_92_0` and `V0_93_0`) are frozen by definition: a shipped schema version
never changes. The other three are the rule's real exceptions, all one shape:
an out-of-crate `_` arm would be *silently* wrong. `quillmark_pdf::FieldType`
is dispatched over whole by `pdfform`'s value resolver and its content-stream
flattener, and a variant they miss draws nothing on the page and reports
nothing. `KnownIslandType` is dispatched over by the typst backend's two
emitters, where a missed type drops the island out of the projection.
`Fidelity` is a ladder a consumer reads to decide what to warn about, and it
has no safe rung to fall through to. For all three the compile error is the
guardrail, and adding a member is a major bump on purpose.
