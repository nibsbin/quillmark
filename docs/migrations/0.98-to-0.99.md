...
| An opaque payload nesting past 128 levels rejected on decode | TypeScript (`install` / `applyChange`; a backstop under the existing 100-level cap on `makeCard` / `insertCard`), Rust (`serial::from_canonical_value`, the op wire) | Nothing — no storable blob reaches the limit |
...
which names the offending field: `"makeCard: field \`tree\` nests deeper than
