...
and seed reads WASM already had. WASM `Document` also gains `getContent(addr)`,
`cardKinds`, and `revision` — see below.

## Every WASM error now carries a code

`Diagnostic.code` is the machine-routable identity of a failure, and every core
and backend producer stamped one — but the binding's own argument errors did
not. Those are the errors a *caller mistake* produces, so they are both the ones
a consumer hits most and the only ones it had to match on English message text.

The `binding::*` namespace fills it: `binding::invalid_argument` (a value the
signature cannot accept), `binding::invalid_dto` (`fromJson` / `loadJson`),
`binding::invalid_content`, `binding::serialize`, `binding::canvas`. The uncoded
door is floored at `invalid_argument`, so nothing reaches a consumer without a
code, and a core producer's code is never overwritten — an out-of-range card
still throws `edit::index_out_of_range` across the boundary.

```js
try { doc.loadJson(blob) } catch (e) {
  if (e.diagnostics[0].code === 'binding::invalid_dto') showFormatHelp()
}
```

Nothing to do unless you were matching these on message text, which was the only
option before.

## Three reads for a document a host holds open

A host that keeps one `Document` across an editing session was reaching for
three answers the surface made expensive or impossible to get.

**`getContent(addr)`** is the read twin of `install(addr, content)`: the content
model, whichever entry point wrote the value. `getStored` is *transport* — its
result shape says how a value was written, so a richtext field reads back as a
content object when a typed write landed it and as the markdown string it parsed
when the document came from `fromMarkdown`. Both are the design, and a codec
consumer had no read that flattened them: `reader.get` projects richtext to
markdown, which is lossy. `getContent` decodes through the same
object-or-markdown dispatch the writer commits, so content round-trips
losslessly and a parsed string imports.

```js
const doc = Document.fromMarkdown(md)
doc.getStored('subject')    // 'Hello *world*'  — the parsed markdown, verbatim
doc.getContent('subject')   // { text: 'Hello world', marks: [...], ... }
```

An absent field is `undefined`; a value that is neither content nor importable
markdown throws `edit::field_richtext_decode`. Like every read on `Document` it
is quill-free, so the caller names a field it knows is richtext — as it does
when writing.

**`cardKinds`** is the card-tree shape without the card bodies: each composable
card's `$kind` in document order, positionally total against `cards`. Reach for
it where `cards` was serializing every payload and body to answer the same
question.

**`revision`** counts edits taken against a handle, monotonic from 0. Every
mutator bumps it, including `loadJson`; no read does. It counts a handle's
history rather than a document's identity, so it compares only against itself —
`clone` restarts at 0, and two handles holding equal documents may sit at
different revisions. A mutator that bumps and then fails its own validation
still counts, which makes the number an upper bound on real changes and never a
lower one: keyed on it, a consumer can do redundant work but cannot miss a
change.

```js
const seen = doc.revision
// … hand the document to arbitrary code …
if (doc.revision !== seen) recompute()
```
