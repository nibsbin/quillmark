...
The release's other break, unrelated to the gate. `quillmark render`'s progress lines (`Loading quill from: …` through `Rendering completed successfully`) go to stderr, where the warning printer already wrote. Under `--stdout` they framed the artifact:
...
A script reading those lines redirects stderr (`2>`) rather than stdout. Only `render` moves: `validate --verbose` still writes its detail lines to stdout, as does `Output written to: <path>`, which prints only when the artifact went to a file.
