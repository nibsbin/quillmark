...
`@quillmark/wasm` exposes exactly one entry in its `exports` map, so there is no subpath around the gate either.
...
5. Leave `Engine`, `MAIN_CARD_ADDR`, `isQuillmarkError` and the guards as static imports.
